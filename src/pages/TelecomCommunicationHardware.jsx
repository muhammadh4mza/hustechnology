import React from 'react';
import Navbar from '../components/Navbar';
import Footer from '../components/Footer';
import TelcomBanner from '../components/TelcomBanner';
import TelcomServices from '../components/TelcomServices';
import TelcomAbout from '../components/TelcomAbout';
import PricingPlansSection from '../components/PricingPlansSection';
import FAQSection from '../components/FAQSection';



const TelecomCommunicationHardware = () => {
  return (
    <div className="relative bg-[#F5F5F5] min-h-screen overflow-hidden">
      {/* Background elements with lower z-index */}
      <div className="absolute inset-0 bg-gradient-to-b from-gray-900 to-gray-800 z-0" />
      <div className="absolute inset-0 opacity-5 z-0">
        <div className="h-full w-full bg-[url('data:image/svg+xml;base64,')] bg-repeat" />
      </div>

      {/* Content with higher z-index */}
      <div className="relative z-10">
        <Navbar />
        <TelcomBanner />
        <TelcomServices />
        <TelcomAbout />
        <PricingPlansSection />
        <FAQSection />

        {/* <TradeStats /> */}
        {/* <IpServicesMeet />
        <IpServicesSection />
        <IpListServicesSection /> */}

        <Footer />
      </div>
    </div>
  );
};

export default TelecomCommunicationHardware;